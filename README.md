...
基于多 Agent 架构的 AI 教学系统。把一个学习目标展开成知识依赖图谱，然后按
「生成计划 → 逐步讲解 → 出题 → 评分 → 下一步」的闭环推进，每个节点的掌握度
随测验结果变化。
...
| 模式 | 输入 | 特点 |
| --- | --- | --- |
| 课程知识库 | 选一门内置课程 | 教材优先，回答标出处 |
| 主题 | 任意主题 | 自由学习 |
| 文档 | 上传 PDF | 严格围绕原文，不超范围 |
| 项目 | 项目需求 | 生成完成该项目所需的技能路径 |

模型支持 Kimi K3（默认）、Gemini、智谱 GLM、通义千问，以及任意 OpenAI 兼容端点。

## 核心特性

### 知识星图

按学习目标生成知识依赖图谱，用 AntV G6 渲染。

- 2D / 3D 力导向视图切换，2D 下可切纵向/横向布局
- 点击节点高亮关联路径；节点填充色表示当前掌握度
- 手动加节点时，AI 会补出中间过渡节点并接上层次连接
- 生成时可选 3 档深度：简洁 4~7 / 标准 8~12 / 详细 13~20 节点
- 界面支持中英文切换

### 自适应教学

- 按掌握度（0%~100%）分 5 档调整讲解深度、代码要求、表达方式和术语密度
- 每个知识点先生成 3–6 步教学计划，按步推进
- 支持图片上传，可识别截图里的代码和公式
- 内置在线 IDE，支持 Python / JavaScript / Go / C / C++ / Java，代码在
  bubblewrap 沙箱里执行（见「在线 IDE 的代码沙箱」）

### 教学闭环

Plan → Teach → Quiz → Evaluate → Next：

1. 按知识点和前驱依赖生成 3–6 步教学计划
2. 每步只讲当前步骤内容
3. 每步讲完由独立的评估 Agent 出题
4. 步骤分独立记录，全局掌握度按加权聚合（后面步骤权重更大）
5. 答错可基于错误分析重讲，通过后进入下一步

测验题绑定教学计划版本、当前步骤和最近一次完整讲解，旧题或串步骤的题会被拒绝。

### 学习画像

- 左侧仪表板显示进度曲线，可折叠
- 每步测验分记在 `step_scores`，全局掌握度另算（见下方评分算法）
- 对话、节点状态、教学计划和步骤分都会持久化
- 主页右侧列出最近星图，可恢复到上次的节点、步骤、聊天和未完成的测验

### 流式输出

- 自由问答、开始讲课、下一步、重新讲解都走 SSE 流式返回
- 自由问答可选 1024 / 2048 / 4096 / 8192，或 256~32768 自定义 Max Tokens
- 支持 Thinking 的模型会把思考内容放在可折叠区域，不支持时自动降级并提示

### 星图生成任务

星图生成跑在服务端的任务里，不占用长连接：

- 首页有生成任务列表，实时显示阶段、模型的思考和输出
- 刷新页面能接回进度，反代的读超时不会把生成掐断
- 失败时直接显示模型或网络给出的原始原因

### 主题与阅读

- 暗色主题（默认）与米黄色护眼主题，一键切换
- 面板宽度可拖拽

### 联网搜索

- 教学和讨论时通过 DuckDuckGo 检索资料，回复下方展示可点击的来源
- 生成星图和扩展节点前先检索一次知识结构
- 默认启用，`.env` 里设 `ASTRA_WEB_SEARCH_ENABLED=false` 关闭
- 搜索失败自动回退到无搜索模式
...
| Agent | 职责 |
| --- | --- |
| Knowledge | 构建知识图谱结构，按 3 档复杂度切换提示词 |
| Doc Graph | 基于上传文档构建文档专属星图 |
| Teacher | 按教学计划逐步输出讲解（不出题） |
| Evaluation | 出题、评分与错误分析 |
| Code Runner | 在沙箱里执行使用者提交的代码 |

### 文档模式

上传 PDF（论文、教材、技术文档），教学严格围绕原文。

- PyMuPDF 提取文本，按段落/章节分块，保留页码和标题
- 生成文档专属星图，每个节点关联原文分块
- 教学、出题、评分都必须引用原文
- 拖拽或点击上传，单文件最大 50MB
...
输入想做的项目，生成完成它所需的技能路径星图。节点是实战技能而非发散的概念；
核心技能权重大、辅助技能权重小；教学和测验全程注入项目背景。

### 课程知识库（RAG）

- 教学计划、讲解、讨论、出题、评分、重讲都会先检索当前课程教材
- 引用带文档标题、章节路径、原文摘录和行号
- 默认本地 BM25 中文检索，不依赖外部向量服务；配置 Embedding 后自动启用
  BM25 + 向量混合召回并做排序融合
- 非教材内容标记为「扩展知识」
- 每门课一个独立目录和配置文件，索引、检索、学习状态按 `course_id` 隔离

### 账号与数据

- `POST /api/auth/register`、`POST /api/auth/login`，用户名或邮箱均可登录
- 登录返回 Bearer 令牌，服务端只存令牌的 SHA-256 摘要
- 密码用 PBKDF2-HMAC-SHA256（24 万次迭代）加随机盐，连续失败临时锁定账号
- 可查看/修改资料、改密码、查看与吊销登录会话、注销账号
- SQLite 单文件存储，学习快照按账号隔离，删号级联清理
...
`completion_factor` 的作用是避免「只做完前两步就拿到高掌握度」。
...
5.  **逐步学习**:
    - 每步按当前掌握度选择讲解深度
    - 讲完后点「考一下这步」进入测验，或点「再讲一遍这步」重讲
    - 答对后点「下一步」；最后一步显示为「学完这节」
    - 答错可点「照着错的地方再讲」
...
│   └── doc_graph_agent.py        # 文档模式星图 Agent
├── backend/                 # FastAPI 后端核心代码
│   ├── api.py                 # 主题模式 API 路由
│   ├── doc_api.py             # 文档模式 API 路由
│   ├── course_api.py          # 课程目录、索引状态与检索 API
│   ├── course_runtime.py      # 课程索引构建状态机与并发去重
│   ├── session_api.py         # 历史学习快照 API
│   ├── auth_api.py            # 登录、注册与账号管理 API
│   ├── user_data_api.py       # 账号维度的学习数据存储 API
│   ├── dependencies.py        # Bearer 令牌鉴权依赖
│   ├── app.py                 # 应用入口与统一 409 恢复契约
│   └── models.py              # Pydantic 数据模型
├── core/                    # 核心逻辑
│   ├── prompts.py             # 主题模式 5 档教学/评分提示词
│   ├── doc_prompts.py         # 文档模式专用提示词
...
│   ├── database.py            # SQLite 连接、事务与建表迁移
│   ├── security.py            # 密码散列与令牌生成
│   ├── account_service.py     # 注册、登录、令牌与账号管理
│   ├── user_data_repository.py# 账号维度的学习快照存储
│   ├── streaming_service.py   # SSE 事件编码与空闲心跳
│   ├── graph_jobs.py          # 星图生成任务（进度与模型输出的事件流）
│   ├── sandbox.py             # bubblewrap 沙箱
│   ├── bootstrap_admin.py     # 建出第一个管理员的命令行
│   └── code_runner.py         # 在沙箱里执行使用者代码
├── utils/                   # 工具模块
